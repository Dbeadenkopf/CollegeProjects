/* File: driver.cpp
   ...
   .. will get out the hash statistics and 
   .. be the program that runs the main statistics
   .. of the hash table
*/
...
//
double getmsecs() {
  struct rusage use ;  // struct for re usage
  struct timeval utime ; // struct for time value
...
int myRand(int seed=0) {
  ...
  if (seed) savedSeed = seed ; // give the save seed 
...
void test(int size, int reps){
  ...
  for (int i=0 ; i < reps ; i++) {
    ...
    do { 
      index = myRand() % numWords; // numwords is inside words.h
    ...
    } catch (PinballHashFull &e) {  // try and catching to invoke throwing the exception
      cout << e.what() << endl ;
      break ;
    } catch (...) {
      cout << "Unknown error\n" ;
...
int main() {
  ...
  long int ms = tp.tv_sec * 1000 + tp.tv_usec / 1000; // random seed gets wall clock time
  printf("Random seed set to: %ld\n\n", ms);
  myRand(ms) ; // ms random seed gets past to myRand
