// small test driver to test , the hash table
// implementation
...
// words not inserted in hash table
...
int main() {
  ...
  for (int i=0 ; i < 10 ; i++) {
    try {
      // again may throw an exception 
      // if the inserted string is marked as not found
      PH.insert(inWords[i]) ;
	...
	cout << "   PH.at(" << slot << ") = " << PH.at(slot) << endl ;
      ...
      cout << e.what() << endl ;  // try and catch block to invoke exception
      break ;
    } catch (...) {
      cout << "Unknown error\n" ;
    }
  ...
  cout << "\n\n" ;
  PH.printStats() ; // this will callt he statistics function
  ...
  char *str ;
  cout << "\n\n" ;
  cout << "Removing " << inWords[remove1] << endl ; // testing remove function
  // of the hash table
