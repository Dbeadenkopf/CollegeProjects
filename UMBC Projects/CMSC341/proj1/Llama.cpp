...
void Llama<T,LN_SIZE>::dup(){
  ...
  while(current!= NULL){
    ...
    current = current->m_next; // going through the stack
    cerr << topItem << endl;
    //arrayoffset++; // not sure what I was doing here?
  }
}
...
void Llama<T,LN_SIZE>::swap(){
  ...
  try{
    if(size() < 2){
      throw LlamaUnderflow("The stack does not have at least 2 items");
    }
  }
  catch(LlamaUnderflow& zde){
    cerr << zde.what() << endl;// handling excemption
...
void Llama<T,LN_SIZE>::rot(){
  ...
  while(current!= NULL){
    topItem = current->arr[arrayoffset-3]; // go to top of stack and minus by three to
    // get to about three items down from the top
    current->arr[arrayoffset-3] = current->arr[arrayoffset-2]; // swap that item with the second item down from the
    // top
    current->arr[arrayoffset-2] = current->arr[arrayoffset-1]; // swap that second item from the top with the first item
    current->arr[arrayoffset-1] = topItem; // swap that
    current = current->m_next;
  }
}
...
T Llama<T,LN_SIZE>::pop(){
  ...
  while(current!= NULL){
    // it does not index to the next element after
    // poping off and does not get rid of
    // the element within the array after poping
    // overall a very inefficient pop fuction 
    topItem = current->arr[arrayoffset-1]; // go to top item
    current = current->m_next; // loop thorugh linked list
  }
  
  return topItem; // return that top item off the list
}
...
T Llama<T,LN_SIZE>::peek(int offset) const{
  LlamaNode<T, LN_SIZE> *current;
  // b/c it needs to be a pointer that
  // is going to access an array
  current = m_head;
  try{
  ...
  while(current!= NULL){
	...
	// increment the stack counter so
	// it can then eventaully be at the value
	// that the driver is asking peek for
	stackCounter++;
      }
    }
    // return current->arr[stackCounter];
    current = current->m_next; // go to next node in the linked list
  }
  return 0;
}
