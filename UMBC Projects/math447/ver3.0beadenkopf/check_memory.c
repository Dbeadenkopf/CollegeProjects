...
void diag_memory() {
  ...
  mem_usage_vector = allocate_long_int_vector(np);
...
/*
 * Function: allocate_long_vector
 * Inputs: n - length of the vector
 * Outputs: array of length n
 *
 * This function allocates a long integer vector of length n
 */
long *allocate_long_vector (int n)
{
  long *x;

  if ((x = (long*) calloc (n, sizeof(long))) == NULL){
