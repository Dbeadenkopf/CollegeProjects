...
Game::~Game() {
  ...
  //
  for (unsigned int i=0 ; i < m_numPlayers ; i++) {// changed to unsigned
    delete m_players[i] ;
  }
  delete [] m_players ;
  ...
  //
  for (unsigned int i=0 ; i < m_numStock ; i++) {// changed to unsigned 
    delete m_stock[i] ;
  }
  delete [] m_stock ;
  ...
  //
  for (unsigned int i=0 ; i < m_numDiscard ; i++) {// changed error here
    delete m_discard[i] ;
  }
  delete [] m_discard ;
}
...
void Game::startGame(unsigned int seed) {
  ...
  //
  if (m_numStock < 1) {
    cerr << "Missing cards!\n" ; // possible factor this into the cheating implementation of game, player stole a card
    exit(1) ;
  }
...
void Game::dealCards(){
  ...
  for(int i=0 ; i < 5 ; i++) {
    for (unsigned int p=0 ; p < m_numPlayers ; p++){//changed error here
