...
const int POKECOUNT = 151; // -> this need to be altered with as well, each time we catch a pokemon this 
// number needs to go down, once it hits zero and congrajulate the player.
...
// function to calulate the HPs address
// Pre-condition: Takes in vector of all the pokemon
// Post-condition: based on pokemon number, it assigns
// the proper HP
int assignHP(int pokeNum);
